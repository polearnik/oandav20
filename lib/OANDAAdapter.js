...
OandaAdapter.prototype.getAccounts = function (callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/v3/accounts" + (this.username ? "?username=" + this.username : "")
...
OandaAdapter.prototype.getAccountSummary = function (accountId, callback) {
...
OandaAdapter.prototype.getAccountTransactions = function (accountId, select, callback) {

    this._sendRESTRequest({
        method: "GET",
        path: "/v3/accounts/" + accountId +"/transactions" + select
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
...
OandaAdapter.prototype.getAccountsTransactions = function (accountId, transactionId, callback) {
...
// streamTransact
OandaAdapter.prototype.getStreamTransact = function (accountId, callback) {
    this._sendRESTRequest({
        method: "GET",
...
OandaAdapter.prototype.getPrice = function (symbol, login, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/v3/accounts/"+login+"/pricing?instruments=" + symbol
    }, function (error, body, statusCode) {
        if (error) {
            if (body && body.message) {
                console.error("[ERROR] Response from Oanda", statusCode + " Error: " + body.message + " (OANDA error code " + body.code + ")");
...
OandaAdapter.prototype._streamPrices = function (accountId) {
    ...
    this.pricesRequest = httpClient.sendRequest({
            hostname: this.streamHost,
            method: "GET",
            path: "/v3/prices?accountId=" + accountId + "&instruments=" + this.priceSubscriptions,
            headers: {
                Authorization: "Bearer " + this.accessToken,
                Connection: "Keep-Alive"
            },
...
OandaAdapter.prototype.getCandles = function (symbol, start, end, granularity, callback) {
    ...
    this._sendRESTRequest({
        method: "GET",
        path: "/v3/candles?" + querystring.stringify({
            instrument: symbol,
...
//--------------------детали ордера
OandaAdapter.prototype.getDetOrdersAccount = function (accountId,orderId,callback) {
...
OandaAdapter.prototype.replaceTrade = function (accountId, tradesSpecial, callback) {
    this._sendRESTRequest({
        method: "PUT",
        path: "/v3/accounts/" + accountId + "/trades/"+tradesSpecial+'/orders',
        data: order,
        headers: {
            Authorization: "Bearer " + this.accessToken,
           // "Content-Type": "application/x-www-form-urlencoded"
...
OandaAdapter.prototype.closeTrade = function (accountId, tradeId, orderClose, callback) {

    this._sendRESTRequest({
        //method: "DELETE",
        method: "PUT",
          data: orderClose,
        path: "/v3/accounts/" + accountId + "/trades/" + tradeId +"/close",
         headers: {
            Authorization: "Bearer " + this.accessToken,
        //    "Content-Type": "application/x-www-form-urlencoded"
             "Content-Type": "application/json"
